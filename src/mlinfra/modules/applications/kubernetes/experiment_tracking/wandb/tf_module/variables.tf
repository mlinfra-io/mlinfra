...
variable "service_account_name" {
  type        = string
  description = "The name of the service account to use for wandb"
  default     = "wandb-sa"
}

variable "remote_tracking" {
  type        = bool
  description = "This variable tracks if the wandb is deployed with an external metadata tracker and blob storage"
  default     = false
}

variable "wandb_data_bucket_name" {
  type        = string
  description = ""
  default     = "wandb-data-bucket"
}
...
variable "wandb_chart_version" {
  type        = string
  description = "wandb Chart version."
  default     = "0.3.7"
}
...
variable "wandb_secret" {
  type        = string
  description = "The name of the secret containing the wandb credentials"
  default     = "wandb-db-auth"
}

variable "resources" {
  type        = map(any)
  description = "The resources to allocate to each wandb pod"
  default = {
    requests = {
      cpu    = "100m"
      memory = "1024Mi"
...
variable "nodeSelector" {
  type        = map(any)
  description = "The nodeSelector to schedule wandb pods on specific nodes"
  validation {
    condition = (
      alltrue([
...
variable "tolerations" {
  type        = list(any)
  description = "The tolerations to schedule wandb pods on specific nodes"
  validation {
    condition = (
      alltrue([
...
variable "affinity" {
  type        = map(any)
  description = "The affinity to schedule wandb pods on specific nodes"
  validation {
    condition = (
      alltrue([
